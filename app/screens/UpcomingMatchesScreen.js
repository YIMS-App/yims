...
export default function UpcomingMatchesScreen (props) {
    ...
    const OpenURLButton = ({ url, buttonStyle, textStyle, text }) => {
      ...
      return (<TouchableOpacity style={buttonStyle} onPress={handlePress}>
    ...
    <View style={styles.container}>
       {loading ? 
        <View style={[styles.container, {justifyContent: 'center', alignItems: 'center', flex:1}]}>
          <NavBar navigation={props.navigation} title={"Upcoming Matches"} color={'#3159C4'} extraData = {props.extraData}/>
          <View style={[styles.container, {justifyContent: 'center', alignItems: 'center', flex:1}]}>
        ...
        <View>
          ...
          <View style={styles.matchesContainer}>
              ...
              <FlatList
                ...
                renderItem={(itemData) => {
                  const event = {
                    title: itemData.item.college1 + ' vs. ' + itemData.item.college2 + ": " + itemData.item.sport,
                    description: itemData.item.college1 + ' and ' + itemData.item.college2 + " face off in a game of " + itemData.item.sport,
                  ...
                  const startDateString = startDateObj.toLocaleString('en-US', {hour: 'numeric', minute: 'numeric'});
                  const endDateString = endDateObj.toLocaleString('en-US', {hour: 'numeric', minute:'numeric'});
                  return (
                    <TouchableOpacity onPress={() => {handleSelectedMatch(itemData.item)}}>
                      <View style={styles.futureMatchContainer}>
                        <Text style={styles.futureMatchDate}>{itemData.item.startTime.slice(5, 10)}: </Text>
                        <Text style={styles.futureMatch}>{itemData.item.college1Abbrev} vs. {itemData.item.college2Abbrev} @ {startDateString} - {endDateString} </Text>
                      ...
                      </View>
                      {
                        selectedMatch == itemData.item ? 
                          <View style={styles.matchDropdown}>
                            ...
                            <Text style={styles.locationText}>{itemData.item.location ? itemData.item.location : 'Payne Whitney'}</Text>
                            <OpenURLButton
                            url={link}
